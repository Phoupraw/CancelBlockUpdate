...
public void generateTranslations(TranslationBuilder b) {
    ...
    b.add("modmenu.descriptionTranslation." + CancelBlockUpdate.MOD_ID, "阻止方块更新、方块计划刻、随机刻、放置可行性检查、流体计划刻、铁轨自动转角。在世界生成时启用本模组的效果可能会产生大量浮空方块。可以用游戏规则" + CBUGameRules.KEY_OFF.getName() + "开关效果。");
    b.add(CBUGameRules.KEY_OFF.getTranslationKey(), modName + "：禁用模组全部效果");
    b.add(CBUGameRules.KEY_REPL.getTranslationKey(), modName + "：允许放置方块时替换草和蕨等");
}
